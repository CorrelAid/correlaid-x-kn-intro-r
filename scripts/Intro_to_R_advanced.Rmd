...
l<- c(26, 24, 49.8, 100)
vanhanen_df<-data.frame(p,l, row.names = c("Deutschland", "France","Russia","North Korea"))
...
5. Definiere nun einen zweiten Vektor mit 5 Werten deiner Wahl:
```{r}
vektor2 <- c(_,_,_,_,_)
```

6. Programmiere nun eine Funktion, um die Kovarianz von Vektor1 und Vektor2 zu errechnen:
```{r}
...
```
7. Abschließend willst du aber auch die Korrelation deiner Vektoren wissen, programmiere auch hierfür eine Funktion:
```{r}
...
```{r}
penguin_data <- read_delim(here::here("scripts/penguin_data.csv"), delim = ";")
```
