...
Übung: (Muss definitiv nochmal gegengecheckt/überarbeitet werden)

Jetzt seid ihr gefragt: 

Programmiert eine Funktion, die 5 Werte addiert:
```{r}
...
Nutzt die vorherige Funktion, um eine neue Funktion zu programmieren, die den Durchschnitt der 5 Werte herausgibt
```{r}
...
Definierie nun einen Vektor mit 5 Werten deiner Wahl:
```{r}
vektor1 <- c(_,_,_,_,_)
```

Programmiere nun eine Funktion, die die Varianz des soeben definierten Vektors berrechnet:
```{r}
_______ <- function (a){
  sum((_-mean(_))^2)/5
...
Definiere nun einen zweiten Vektor mit 5 Werten deiner Wahl:
```{r}
vektor2 <- c(_,_,_,_,_)
```

Programmiere nun eine Funktion, um die Kovarianz von Vektor1 und Vektor2 zu errechnen:
```{r}
_______ <-function(a,b){
  sum((_-mean(_)*(_-mean(_))))/5
}
# Test:
_________(______,_______)
```
Abschließend willst du aber auch die Korrelation deiner Vektoren wissen, programmiere auch hierfür eine Funktion:
```{r}
_________ <-function(a,b){
  (________*5)/(sqrt((sum((_-mean(_))^2))/5)*sqrt((sum((_-mean(_))^2))/5))
}
...
```{r}
broom::tidy(lm(body_mass_g ~ flipper_length_mm, data = penguins))
```
